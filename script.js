  ...
  grid.innerHTML = offers.map(p => `
    <div class="offer-card">
      ...
      <div class="offer-info">
        ...
        ${cardExtras(p)}
        <button class="offer-btn" onclick="window.open('${p.affiliate_link}','_blank')">Ver Oferta ↗</button>
      </div>
    </div>
  `).join('');
...
window.openModal = function(p) {
    ...
    modal.innerHTML = `
      ...
      <div class="modal-box">
        ...
        <div class="modal-body">
          ...
          <button class="modal-btn" id="modal-btn">Ver Oferta ↗</button>
        </div>
      </div>`;
  ...
  $('modal-btn').onclick = () => window.open(p.affiliate_link, '_blank');
  ...
  grid.innerHTML = filtered.map((p, i) => `
    <div class="product-card" style="animation-delay:${(i % 10) * 0.04}s">
      ...
      <div class="product-info">
        ...
        ${cardExtras(p)}
        <button class="product-btn" onclick="window.open('${p.affiliate_link}','_blank')">Ver Oferta ↗</button>
      </div>
    </div>
  `).join('');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initNavLinks();
  initHero();
  initStoreCards();   /* ← marca ML como ativo visualmente */
  loadAndRender();    /* ← carrega produtos já filtrando por ML */
});
