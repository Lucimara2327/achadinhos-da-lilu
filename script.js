...
async function fetchProducts() {
  ...
  const CACHE_TIME_KEY = 'lilu_products_cache_time';
  const CACHE_DURATION = 5 * 60 * 1000; // 5 minutos

  // Tenta usar cache se for recente
  try {
...
async function fetchFreshProducts() {
  try {
    ...
    try {
      localStorage.setItem('lilu_products_cache', JSON.stringify(data));
      localStorage.setItem('lilu_products_cache_time', Date.now().toString());
    } catch(e) { /* armazenamento cheio, ignora cache */ }
    // Se já tinha renderizado com cache antigo, re-renderiza com dados frescos
    ...
    <div class="product-card" style="animation-delay:${(i % 10) * 0.04}s">
      ...
      <div class="product-info">
        ...
        <div style="display:flex;gap:6px;margin-top:auto;">
          <button class="product-btn" style="flex:1" onclick="event.stopPropagation();trackClick(${JSON.stringify(p).replace(/"/g, '&quot;')});window.open('${p.affiliate_link}','_blank')">Ver Oferta ↗</button>
